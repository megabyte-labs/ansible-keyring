...
# @description This script will install Ansible Keyring. The script heavily borrows from
...
#
#   ```
#   curl -sSL https://raw.githubusercontent.com/megabyte-labs/ansible-keyring/master/local/install.sh | sh
#   ```

RELEASES_URL="https://github.com/megabyte-labs/ansible-keyring/releases"
...
if [ "${OS}" != "linux" ] && { [ "${ARCH}" = "ppc64" ] || [ "${ARCH}" = "ppc64le" ];}; then
  ...
  BINARY="ansible-keyring-${OS}-${ARCH}"
  if [ "$OS" = "windows" ]; then
    BINARY="$BINARY.exe"
  fi
fi
